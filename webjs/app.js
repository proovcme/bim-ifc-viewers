...
class BIMApp {
    ...
    _bindEvents(){
        ...
        on('btn-parse-spec',()=>this._parseSpec());
        on('btn-export-csv',()=>this._exportSpecCSV());
    ...
    async _parseSpec(){
        if(!Object.keys(this.bimData).length){ this._status('Сначала загрузите модель'); return; }
        const wrap=document.getElementById('spec-table-wrap');
        if(!wrap)return;
        // Рендерим список категорий сразу — без Qto
        this._specData={};
        let html='';
        for(const cat of BIM_CATS){
            const entry=this.bimData[cat.key];
            if(!entry||!entry.count)continue;
            html+=`<div class="spec-cat-block" data-key="${cat.key}">`
                +`<div class="spec-cat-hdr">`
                +`<span class="spec-cat-title">${cat.icon} ${cat.label}</span>`
                +`<span class="spec-cat-cnt">${entry.count} шт</span>`
                +`<button class="btn spec-calc-btn" data-key="${cat.key}">Считать</button>`
                +`</div>`
                +`<div class="spec-qto-row" id="spec-qto-${cat.key}" style="display:none"></div>`
                +'</div>';
        }
        wrap.innerHTML=html||'<p class="sys-empty">Нет категорий</p>';
        wrap.querySelectorAll('.spec-calc-btn').forEach(btn=>{
            btn.addEventListener('click',async()=>{
                const key=btn.dataset.key;
                btn.disabled=true; btn.textContent='⏳';
                await this._calcCatQto(key);
                btn.textContent='↻';
                btn.disabled=false;
            });
        });
    ...
    async _calcCatQto(key){
        const entry=this.bimData[key];
        if(!entry)return;
        const mgr=this.loader.ifcManager;
        const qto={area:0,length:0,volume:0};
        for(const{mid,ids}of entry.midMap){
            for(const id of ids){
                const sets=await tryIFC(()=>mgr.getPropertySets(mid,id,false),[]);
                for(const ps of sets||[]){
                    if(!_isQset(ps))continue;
                    for(const q of ps.Quantities||ps.HasProperties||[]){
                        if(!q)continue;
                        const av=parseFloat(q.AreaValue?.value??q.AreaValue??'');
                        const lv=parseFloat(q.LengthValue?.value??q.LengthValue??'');
                        const vv=parseFloat(q.VolumeValue?.value??q.VolumeValue??'');
                        if(!isNaN(av))qto.area+=av;
                        if(!isNaN(lv))qto.length+=lv;
                        if(!isNaN(vv))qto.volume+=vv;
                    }
                }
            }
        }
        this._specData[key]=qto;
        const fv=(n,unit)=>n>0.001?n.toFixed(2)+' '+unit:'';
        const row=document.getElementById('spec-qto-'+key);
        if(!row)return;
        const parts=[fv(qto.area,'м²'),fv(qto.length,'м'),fv(qto.volume,'м³')].filter(Boolean);
        if(parts.length){
            row.innerHTML=parts.map(p=>`<span class="spec-qto-val">${p}</span>`).join('')
                +`<button class="btn spec-csv-btn" data-key="${key}" style="margin-left:auto">↓ CSV</button>`;
            row.querySelector('.spec-csv-btn').addEventListener('click',()=>this._exportCatCSV(key));
        } else {
            row.innerHTML='<span class="spec-qto-val" style="color:#aaa">Qto не заданы в модели</span>';
        }
        row.style.display='flex';
    }

    _exportCatCSV(key){
        const cat=BIM_CATS.find(c=>c.key===key);
        const entry=this.bimData[key];
        const qto=this._specData?.[key];
        if(!cat||!entry)return;
        const fv=(n)=>n>0.001?n.toFixed(3):'';
        const rows=[
            ['Категория','Кол-во, шт','Площадь, м²','Длина, м','Объём, м³'],
            [cat.label, entry.count, fv(qto?.area??0), fv(qto?.length??0), fv(qto?.volume??0)]
        ];
        const csv='\uFEFF'+rows.map(r=>r.map(csvEscape).join(',')).join('\r\n');
        const a=document.createElement('a');
        a.href=URL.createObjectURL(new Blob([csv],{type:'text/csv;charset=utf-8'}));
        a.download='BIM_'+cat.label+'_'+new Date().toISOString().slice(0,10)+'.csv';
        a.click();
    }

    _exportSpecCSV(){
        // Экспорт всего что уже посчитано
        const counted=BIM_CATS.filter(c=>this._specData?.[c.key]);
        if(!counted.length){ this._status('Сначала считайте хотя бы одну категорию'); return; }
        const fv=(n)=>n>0.001?n.toFixed(3):'';
        const rows=[
            ['Категория','Кол-во, шт','Площадь, м²','Длина, м','Объём, м³'],
            ...counted.map(c=>{
                const entry=this.bimData[c.key];
                const qto=this._specData[c.key];
                return [c.label, entry.count, fv(qto.area), fv(qto.length), fv(qto.volume)];
            })
        ];
        const csv='\uFEFF'+rows.map(r=>r.map(csvEscape).join(',')).join('\r\n');
        const a=document.createElement('a');
        a.href=URL.createObjectURL(new Blob([csv],{type:'text/csv;charset=utf-8'}));
        a.download='BIM_Spec_'+new Date().toISOString().slice(0,10)+'.csv';
        a.click();
    }
    ...
    async _parseSystems(){
        ...
        for(const[,m]of this.models){
            ...
            // Индекс sysID → [] членов через IFCRELASSIGNSTOPRODUCT
            // (инверсные атрибуты не работают в web-ifc@0.0.39)
            const sysMembers=new Map();
            sysIDs.forEach(id=>sysMembers.set(id,[]));
            const assignIDs=await tryIFC(()=>mgr.getAllItemsOfType(m.modelID,IFCRELASSIGNSTOPRODUCT,false),[]);
            for(const aID of assignIDs||[]){
                const rel=await tryIFC(()=>mgr.getItemProperties(m.modelID,aID));
                if(!rel)continue;
                const sysRef=rel.RelatingProduct?.value;
                if(sysRef==null||!sysMembers.has(sysRef))continue;
                for(const obj of rel.RelatedObjects||[]){
                    if(obj?.value!=null)sysMembers.get(sysRef).push(obj.value);
                }
            }
            ...
            for(const sysID of sysIDs){
                ...
                const memberIDs=sysMembers.get(sysID)||[];
